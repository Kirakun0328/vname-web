...
for(const [jp,values] of Object.entries({"ぶいネームの更新情報も、このアカウントでお知らせします。":["Updates about ぶいネーム are also shared on this account.","本账号也会发布ぶいネーム的更新消息。","이 계정에서 ぶいネーム 업데이트 소식도 전해 드립니다."],"更新情報をXで見る":["See updates on X","在X查看更新消息","X에서 업데이트 소식 보기"],"フォローは任意です。申請・掲載の条件ではありません。":["Following is optional and is not required to request or receive a listing.","关注为自愿，不是申请或收录的条件。","팔로우는 선택 사항이며 신청·등재 조건이 아닙니다."]})) ['en','zh','ko'].forEach((lang,i)=>translations[lang][jp]=values[i]);

for(const [jp,values] of Object.entries({"掲載・修正":["Listings & corrections","收录与更正","등재·수정"],"あなたの活動名を載せよう":["Add your activity name","收录你的活动名","활동명을 등록해 보세요"],"① 活動名":["① Activity name","① 活动名","① 활동명"],"② 読み":["② Pronunciation","② 读音","② 읽는 법"],"③ 活動URL":["③ Profile URL","③ 活动URL","③ 활동 URL"],"申請前に確認":["Before you request","申请前须知","신청 전 확인"],"更新・応援":["Updates & support","更新与支持","업데이트·응원"],"ぶいネームを広めてね":["Help spread the word","帮忙推广ぶいネーム","ぶいネーム을 알려 주세요"],"いいね・RPで応援してもらえると、とても嬉しいです！":["Your likes and reposts would make me very happy!","如果能点赞、转发支持，我会非常开心！","좋아요와 리포스트로 응원해 주시면 정말 기뻐요!"],"いいね・RPで応援":["Support with likes & reposts","点赞转发支持","좋아요·리포스트로 응원"],"掲載・修正の申請":["Request a listing or correction","申请收录或更正","등재·수정 신청"],"フォロー・いいね・RPは任意です。申請・掲載の条件ではありません。":["Following, liking and reposting are optional and are not conditions for requesting or receiving a listing.","关注、点赞和转发均为自愿，不是申请或收录的条件。","팔로우·좋아요·리포스트는 선택 사항이며 신청·등재 조건이 아닙니다."]})) ['en','zh','ko'].forEach((lang,i)=>translations[lang][jp]=values[i]);

for(const [jp,values] of Object.entries({"掲載・読み方の修正":["Listings & pronunciation corrections","收录与读音更正","등재·읽는 법 수정"],"追加・読み方の修正はこちら":["Request an addition or pronunciation fix","申请添加或更正读音","추가·읽는 법 수정 신청"],"② 正しい読み":["② Correct pronunciation","② 正确读音","② 올바른 읽는 법"],"読み方の修正は、本人・公式の読みが分かるURLもあると助かります。":["For pronunciation corrections, a link showing the creator's or official pronunciation helps.","更正读音时，如能附上本人或官方标明读音的链接，将有助于确认。","읽는 법 수정 시 본인이나 공식 자료에서 발음을 확인할 수 있는 URL도 보내 주시면 도움이 됩니다."],"時間が取れたときに、まとめて確認・反映します。自動反映ではありません。":["Requests are reviewed and applied in batches when time allows, not automatically.","有空时会集中确认并更新，不会自动反映。","시간이 날 때 모아서 확인·반영합니다. 자동 반영은 아닙니다."]})) ['en','zh','ko'].forEach((lang,i)=>translations[lang][jp]=values[i]);

for(const [jp,values] of Object.entries({"応援":["Support","支持","응원"],"フォロー・RP・いいねで応援してね！":["Support with follows, reposts and likes!","欢迎关注、转发、点赞支持！","팔로우·리포스트·좋아요로 응원해 주세요!"],"開発者のXで応援してもらえると、とても嬉しいです！":["Your support on the developer’s X account would make me very happy!","如果能在开发者的X账号上给予支持，我会非常开心！","개발자의 X 계정에서 응원해 주시면 정말 기뻐요!"],"きらっちのXへ":["Visit Kiratchi on X","前往きらっち的X","きらっち의 X로"]})) ['en','zh','ko'].forEach((lang,i)=>translations[lang][jp]=values[i]);

for(const [jp,values] of Object.entries({"フォロー・RP・いいねで応援してもらえると嬉しいです！":["Your follows, reposts and likes would make me very happy!","如果能关注、转发、点赞支持，我会非常开心！","팔로우·리포스트·좋아요로 응원해 주시면 정말 기뻐요!"]})) ['en','zh','ko'].forEach((lang,i)=>translations[lang][jp]=values[i]);

for(const [jp,values] of Object.entries({"開発者のツール紹介":["Tools by the developer","开发者的工具","개발자의 도구"],"配信・動画づくりに使えるツール":["Tools for streams and videos","直播与视频创作工具","방송·영상 제작 도구"],"言葉から、アバターの動きを。":["Turn words into avatar motion.","用文字，让虚拟形象动起来。","말로 만드는 아바타 모션."],"「手を振る」「ジャンプする」などのテキストから、AIでVRMアニメーションを生成。作った動きをVRMA形式で保存できます。":["Generate VRM animations with AI from prompts like “wave” or “jump,” then save the motion in VRMA format.","输入“挥手”“跳跃”等文字，让AI生成VRM动画，并保存为VRMA格式。","“손 흔들기”, “점프하기” 같은 텍스트로 AI가 VRM 애니메이션을 생성합니다. 만든 동작을 VRMA 형식으로 저장할 수 있어요."],"詳しく見る・ダウンロード":["Learn more / Download","了解详情・下载","자세히 보기·다운로드"]})) ['en','zh','ko'].forEach((lang,i)=>translations[lang][jp]=values[i]);
